...
class _ReportHistoryScreenState extends State<ReportHistoryScreen> {
  ...
  String _safeFileName(TyreRecord r) {
    final base = r.vehicleModel.trim().isNotEmpty
        ? r.vehicleModel.trim()
        : '${r.vehicleType}_${r.recordId}';
    final model = _cleanFileNamePart(base, fallback: 'Tyre_Report_${r.recordId}');
    return '$model.pdf';
  }
  ...
  void _openDetailsSheet(TyreRecord record) {
    final s = MediaQuery.sizeOf(context).width / 390.0;
              ...
              children: [
                ...
                ...filtered.map((it) {
                  ...
                  return Padding(
                    ...
                    child: _ReportCard(
                      vehicleModel: it.vehicleModel.trim().isEmpty ? (it.vehicleId.trim().isEmpty ? 'Record #${it.recordId}' : it.vehicleId) : it.vehicleModel,
                      s: s,
                      ...
                      onViewDetails: () => _openDetailsSheet(it),
                    ),
                  );
                }),
              ],
...
class _ModernPdfReport {
  ...
  static String _mapValue(dynamic obj, String key) {
    final m = _asMap(obj);
    return _dash(m == null ? null : m[key]);
  }

  static String _sidewallDamageStatus(dynamic sidewall) {
    final m = _asMap(sidewall);
    final dmg = _asMap(m == null ? null : m['sidewall_damage']);
    return _dash(dmg == null ? null : dmg['status']);
  }

  static String _sidewallDamageDescription(dynamic sidewall) {
    final m = _asMap(sidewall);
    final dmg = _asMap(m == null ? null : m['sidewall_damage']);
    return _dash(dmg == null ? null : dmg['description']);
  }
  ...
  static double? _treadMmFromString(String raw) {
    ...
    if (m1 != null) {
      ...
      final mm = (num / den) * 25.4;
      return mm.isFinite ? mm : null;
    }

    final mmMatch = RegExp(r'([0-9]+(?:\.[0-9]+)?)\s*mm').firstMatch(s);
    if (mmMatch != null) return double.tryParse(mmMatch.group(1) ?? '');

    final firstNum = RegExp(r'([0-9]+(?:\.[0-9]+)?)').firstMatch(s);
    if (firstNum != null) return double.tryParse(firstNum.group(1) ?? '');
    return null;
  }
  ...
  static PdfColor _treadStatusColor(String status) {
    final t = status.toLowerCase();
    if (t.contains('critical')) return const PdfColor.fromInt(0xFF991B1B);
    if (t.contains('danger')) return const PdfColor.fromInt(0xFFEF4444);
    if (t.contains('warning')) return const PdfColor.fromInt(0xFFF59E0B);
    if (t.contains('safe')) return const PdfColor.fromInt(0xFF22C55E);
    return _text;
  }

  static String _fmtMm(double? mm) => mm == null ? '—' : '${mm.toStringAsFixed(1)} mm';

  static String _pressureStatus(dynamic pressure) => _mapValue(pressure, 'status');
  static String _pressureReason(dynamic pressure) => _mapValue(pressure, 'reason');
  static String _pressureConfidence(dynamic pressure) => _mapValue(pressure, 'confidence');
      ...
      child: pw.Row(
        ...
        children: [
          pw.SizedBox(
            ...
            child: pw.Text(
              k,
              style: pw.TextStyle(fontSize: 9.5, fontWeight: pw.FontWeight.bold, color: _muted),
            ),
          ),
          pw.Expanded(
            child: pw.Text(
              v.trim().isEmpty ? '—' : v.trim(),
              style: pw.TextStyle(fontSize: 10.2, color: vColor ?? _text, height: 1.25),
            ),
          ),
        ],
      ),
  ...
  static pw.Widget _sectionTitle(String title) {
    return pw.Padding(
      ...
      child: pw.Text(
        title,
        style: pw.TextStyle(fontSize: 10.8, fontWeight: pw.FontWeight.bold, color: _text),
      ),
    );
  }
  ...
  static pw.Widget _imageBox(pw.ImageProvider? image) {
    return pw.Container(
      ...
      child: image == null
          ? pw.Center(child: pw.Text('No image', style: pw.TextStyle(fontSize: 9.5, color: _muted)))
          : pw.ClipRRect(
              horizontalRadius: 8,
              verticalRadius: 8,
  ...
  static pw.Widget _tyreTile(_WheelData w) {
    ...
    return pw.Container(
      ...
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          pw.Text(w.title, style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: _text)),
          pw.SizedBox(height: 8),
          _imageBox(w.image),
          pw.SizedBox(height: 10),

          _sectionTitle('Tread Inspection'),
          _kv('AI Tyre Status', _dash(w.status)),
          ...
          _sectionTitle('Tyre Pressure'),
          _kv('Status', _pressureStatus(w.pressure)),
          _kv('Reason', _pressureReason(w.pressure)),
          _kv('Confidence', _pressureConfidence(w.pressure)),

          pw.Divider(color: _border),
          _sectionTitle('Sidewall Details'),
          _kv('Is Tire', _mapValue(w.sidewall, 'is_tire')),
          _kv('Brand', _mapValue(w.sidewall, 'brand')),
          _kv('Model', _mapValue(w.sidewall, 'model')),
          _kv('Size', _mapValue(w.sidewall, 'size')),
          _kv('Width', _mapValue(w.sidewall, 'width')),
          _kv('Aspect Ratio', _mapValue(w.sidewall, 'aspect_ratio')),
          _kv('Rim Diameter', _mapValue(w.sidewall, 'rim_diameter')),
          _kv('Load Index', _mapValue(w.sidewall, 'load_index')),
          _kv('Speed Rating', _mapValue(w.sidewall, 'speed_rating')),
          _kv('Manufacturing Date', _mapValue(w.sidewall, 'manufacturing_date')),
          _kv('Damage Status', _sidewallDamageStatus(w.sidewall)),
          _kv('Damage Detail', _sidewallDamageDescription(w.sidewall)),
          _kv('Sidewall Confidence', _mapValue(w.sidewall, 'confidence')),

          pw.Divider(color: _border),
          _sectionTitle('Summary'),
          pw.Text(_dash(w.summary), style: pw.TextStyle(fontSize: 10, color: _text, height: 1.35)),
        ],
      ),
    );
  }
      ...
      pw.MultiPage(
        ...
        build: (_) => [
          pw.RichText(
            text: pw.TextSpan(
              children: [
                pw.TextSpan(text: 'Tyre ', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: _g1)),
                pw.TextSpan(text: 'Inspection Report', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, color: _g2)),
              ],
            ),
          ),
          ...
          pw.Container(
            padding: const pw.EdgeInsets.all(12),
            decoration: pw.BoxDecoration(color: _card, borderRadius: pw.BorderRadius.circular(12), border: pw.Border.all(color: _border)),
            child: pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                _kv('Record ID', r.recordId == 0 ? '—' : r.recordId.toString()),
                _kv('Uploaded', r.uploadedAt.millisecondsSinceEpoch == 0 ? '—' : DateFormat('dd MMM, yyyy  -  hh:mm a').format(r.uploadedAt)),
                _kv('Vehicle Type', _dash(r.vehicleType.toUpperCase())),
                _kv('Vehicle ID', _dash(r.vehicleId)),
                _kv('VIN', _dash(r.vin)),
          ...
          pw.SizedBox(height: 12),
          ...wheels.map((w) => pw.Padding(
                padding: const pw.EdgeInsets.only(bottom: 12),
                child: _tyreTile(w),
              )),
          pw.SizedBox(height: 18),
          pw.Center(child: pw.Text('Generated by TireTest AI', style: pw.TextStyle(fontSize: 9, color: _muted))),
        ],
      ),
    );
              ...
              child: Column(
                ...
                children: [
                  ...
                  GestureDetector(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.visibility_rounded, size: 16 * s, color: const Color(0xFF4F7BFF)),
                        SizedBox(width: 6 * s),
                        Text(
                          'View full details',
                          style: TextStyle(
...
class _RecordDetailsSheet extends StatelessWidget {
  ...
  static String _read(dynamic source, String key) => _dash(_map(source)[key]);

  static String _damage(dynamic sidewall, String key) {
    final damage = _map(_map(sidewall)['sidewall_damage']);
    return _dash(damage[key]);
  }

  @override
  Widget build(BuildContext context) {
    final isBike = record.vehicleType.toLowerCase().trim() == 'bike';
    final wheels = isBike
        ? <_DetailsWheel>[
            _DetailsWheel(
              title: 'Front Tyre',
              image: record.bikeFrontWheel,
              status: record.bikeFrontStatus,
              tread: record.bikeFrontTread,
              wear: record.bikeFrontWearPatterns,
              pressure: record.bikeFrontPressure,
              summary: record.bikeFrontSummary,
              sidewall: record.bikeFrontSidewall,
            ),
            _DetailsWheel(
              title: 'Back Tyre',
              image: record.bikeBackWheel,
              status: record.bikeBackStatus,
              tread: record.bikeBackTread,
              wear: record.bikeBackWearPatterns,
              pressure: record.bikeBackPressure,
              summary: record.bikeBackSummary,
              sidewall: record.bikeBackSidewall,
            ),
          ]
        : <_DetailsWheel>[
            _DetailsWheel(
              title: 'Front Left Tyre',
              image: record.frontLeftWheel,
              status: record.frontLeftStatus,
              tread: record.frontLeftTread,
              wear: record.frontLeftWearPatterns,
              pressure: record.frontLeftPressure,
              summary: record.frontLeftSummary,
              sidewall: record.frontLeftSidewall,
            ),
            _DetailsWheel(
              title: 'Front Right Tyre',
              image: record.frontRightWheel,
              status: record.frontRightStatus,
              tread: record.frontRightTread,
              wear: record.frontRightWearPatterns,
              pressure: record.frontRightPressure,
              summary: record.frontRightSummary,
              sidewall: record.frontRightSidewall,
            ),
            _DetailsWheel(
              title: 'Back Left Tyre',
              image: record.backLeftWheel,
              status: record.backLeftStatus,
              tread: record.backLeftTread,
              wear: record.backLeftWearPatterns,
              pressure: record.backLeftPressure,
              summary: record.backLeftSummary,
              sidewall: record.backLeftSidewall,
            ),
            _DetailsWheel(
              title: 'Back Right Tyre',
              image: record.backRightWheel,
              status: record.backRightStatus,
              tread: record.backRightTread,
              wear: record.backRightWearPatterns,
              pressure: record.backRightPressure,
              summary: record.backRightSummary,
              sidewall: record.backRightSidewall,
            ),
          ];
      ...
      builder: (context, controller) {
        return Container(
          ...
          child: ListView(
            ...
            padding: EdgeInsets.fromLTRB(16 * s, 12 * s, 16 * s, 26 * s),
            children: [
              Center(
                child: Container(
                  width: 46 * s,
              ...
              Row(
                children: [
                  Expanded(
                    child: Text(
                      'Full ${record.vehicleType.toUpperCase()} Report',
                      style: TextStyle(
                        fontFamily: 'ClashGrotesk',
                        fontSize: 20 * s,
                        fontWeight: FontWeight.w900,
              ...
              _DetailsCard(
                ...
                rows: [
                  _InfoRow('Record ID', record.recordId == 0 ? '—' : record.recordId.toString()),
                  _InfoRow('User ID', record.userId),
                  _InfoRow('Vehicle Type', record.vehicleType),
                  _InfoRow('Vehicle ID', record.vehicleId),
                  _InfoRow('VIN', record.vin),
                  _InfoRow('Uploaded At', record.uploadedAt.millisecondsSinceEpoch == 0 ? '—' : DateFormat('dd MMM, yyyy  —  hh:mm a').format(record.uploadedAt)),
                  _InfoRow('Vehicle Brand', record.vehicleBrand),
                  _InfoRow('Vehicle Model', record.vehicleModel),
                  _InfoRow('Vehicle License', record.vehicleLicense),
                  _InfoRow('Tyre Brand', record.vehicleTyreBrand),
                  _InfoRow('Tyre Dimension', record.vehicleTyreDimension),
                ],
              ),
              SizedBox(height: 12 * s),
              ...wheels.map((w) => Padding(
                    padding: EdgeInsets.only(bottom: 12 * s),
                    child: _WheelDetailsCard(s: s, wheel: w),
                  )),
              SizedBox(height: 4 * s),
              ElevatedButton.icon(
                onPressed: onDownload,
                icon: const Icon(Icons.picture_as_pdf_rounded),
                label: const Text('Save / Share Full PDF'),
...
class _DetailsCard extends StatelessWidget {
  const _DetailsCard({required this.s, required this.title, required this.rows});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: const [BoxShadow(color: Color(0x10000000), blurRadius: 12, offset: Offset(0, 6))],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
...
class _WheelDetailsCard extends StatelessWidget {
  const _WheelDetailsCard({required this.s, required this.wheel});

  final double s;
  final _DetailsWheel wheel;

  @override
  Widget build(BuildContext context) {
    final pMap = _RecordDetailsSheet._map(wheel.pressure);
    final sMap = _RecordDetailsSheet._map(wheel.sidewall);
    final damage = _RecordDetailsSheet._map(sMap['sidewall_damage']);

    return _DetailsCard(
      s: s,
      title: wheel.title,
      rows: [
        _InfoRow('Image URL', wheel.image),
        _InfoRow('Status', wheel.status),
        _InfoRow('Tread Depth', wheel.tread),
        _InfoRow('Wear Patterns', wheel.wear),
        _InfoRow('Pressure Status', _RecordDetailsSheet._dash(pMap['status'])),
        _InfoRow('Pressure Reason', _RecordDetailsSheet._dash(pMap['reason'])),
        _InfoRow('Pressure Confidence', _RecordDetailsSheet._dash(pMap['confidence'])),
        _InfoRow('Summary', wheel.summary),
        _InfoRow('Sidewall Is Tire', _RecordDetailsSheet._dash(sMap['is_tire'])),
        _InfoRow('Sidewall Brand', _RecordDetailsSheet._dash(sMap['brand'])),
        _InfoRow('Sidewall Model', _RecordDetailsSheet._dash(sMap['model'])),
        _InfoRow('Sidewall Size', _RecordDetailsSheet._dash(sMap['size'])),
        _InfoRow('Sidewall Width', _RecordDetailsSheet._dash(sMap['width'])),
        _InfoRow('Aspect Ratio', _RecordDetailsSheet._dash(sMap['aspect_ratio'])),
        _InfoRow('Rim Diameter', _RecordDetailsSheet._dash(sMap['rim_diameter'])),
        _InfoRow('Load Index', _RecordDetailsSheet._dash(sMap['load_index'])),
        _InfoRow('Speed Rating', _RecordDetailsSheet._dash(sMap['speed_rating'])),
        _InfoRow('Manufacturing Date', _RecordDetailsSheet._dash(sMap['manufacturing_date'])),
        _InfoRow('Sidewall Confidence', _RecordDetailsSheet._dash(sMap['confidence'])),
        _InfoRow('Damage Status', _RecordDetailsSheet._dash(damage['status'])),
        _InfoRow('Damage Description', _RecordDetailsSheet._dash(damage['description'])),
      ],
    );
  }
}
